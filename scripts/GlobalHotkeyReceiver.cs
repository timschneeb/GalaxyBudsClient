...
public class GlobalHotkeyReceiver : IHook
{
    ...
    protected ScriptLogger Logger => new ScriptLogger(this);
    
    public async void OnHooked()
    {
        await Task.Delay(5000).ContinueWith((_) => Initialize());
    }
    public void OnUnhooked()
    {
        if (MainWindow.Instance.PlatformImpl is Win32ProcWindowImpl impl)
        {
            ...
            impl.MessageReceived -= OnWndProcMessage;
        }
    }
    ...
    private void Initialize()
    {
        ...
        if (MainWindow.Instance.PlatformImpl is Win32ProcWindowImpl impl)
        {
            Logger.Debug("Initializing WndProc client");
            impl.MessageReceived += OnWndProcMessage;

            if (impl.Dispatcher == null)
            {
                Logger.Error("Abnormal state: Win32Proc dispatcher not yet initialized");
                ScriptManager.Instance.UnregisterHook(this);
                return;
            }
            ...
            impl.Dispatcher.Invoke(() =>
            {
                Hotkeys.ToList().ForEach((e) => RegisterHotKey(e.Item1, e.Item2));
            });
        }
        else
        {
            Logger.Error("This platform is not supported by this script. Hotkeys unavailable.");
            ScriptManager.Instance.UnregisterHook(this);
        }
    }

    private void OnWndProcMessage(object? sender, WndProcClient.WindowMessage msg)
    {      
        if (msg.Msg == WndProcClient.WindowsMessage.WM_HOTKEY)
        {
            ...
            Logger.Debug($"Received event (Modifiers: {modifier}; Key: {key})");

            Hotkeys
                .Where(x => x.Item1 == modifier && x.Item2 == key)
                .ToList()
                .ForEach(x => x.Item3());
        }
    }
    ...
    public void RegisterHotKey(ModifierKeys modifier, Keys key)
    {
        ...
        {
            Logger.Error($"Unable to register hotkey (Error code: {Marshal.GetLastWin32Error()}) (Modifiers: {modifier}; Key: {key})");
        }
        else
        {
            Logger.Info($"Hotkey successfully registered (Modifiers: {modifier}; Key: {key})");
        }
    }
